...
class ProcessManager(QWidget):
	...
	def update_texts(self):
		...
		self.table.setHorizontalHeaderLabels(
			self.lang['columns'] + ["Start Time", "Path", "Threads", "Bytes Sent"]
		)
		...
		for proc in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_percent', 'username']):
			try:
				...
				if search in name.lower():
					...
					self.table.setItem(row, 3, QTableWidgetItem(f"{proc.info['memory_percent']:.1f}"))
					self.table.setItem(row, 4, QTableWidgetItem(proc.info['username']))
					...
					sent = str(net_io.bytes_sent)
					self.table.setItem(row, 8, QTableWidgetItem(sent))
			except:
				continue
